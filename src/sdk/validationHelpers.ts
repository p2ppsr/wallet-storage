...
export function validateInternalizeActionArgs(args: sdk.InternalizeActionArgs): ValidInternalizeActionArgs {
  ...
  for (const o of vargs.outputs)
    if (o.protocol === 'wallet payment') {
      if (vargs.commonDerivationPrefix && o.paymentRemittance!.derivationPrefix !== vargs.commonDerivationPrefix) throw new sdk.WERR_INVALID_PARAMETER('derivationPrefix', 'identical between outputs of the same transaction')
      else if (!vargs.commonDerivationPrefix) vargs.commonDerivationPrefix = o.paymentRemittance!.derivationPrefix
    }
