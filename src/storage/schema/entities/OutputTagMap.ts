...
export class OutputTagMap extends EntityBase<table.OutputTagMap> {
  ...
  override equals(ei: table.OutputTagMap, syncMap?: entity.SyncMap | undefined): boolean {
    ...
    if (eo.outputId !== (syncMap ? syncMap.output.idMap[verifyId(ei.outputId)] : ei.outputId) || eo.outputTagId !== (syncMap ? syncMap.outputTag.idMap[verifyId(ei.outputTagId)] : ei.outputTagId) || eo.isDeleted !== ei.isDeleted) return false

    return true
  }

  static async mergeFind(storage: entity.EntityStorage, userId: number, ei: table.OutputTagMap, syncMap: entity.SyncMap, trx?: sdk.TrxToken): Promise<{ found: boolean; eo: entity.OutputTagMap; eiId: number }> {
