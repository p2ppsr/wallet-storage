...
export class TxLabelMap extends entity.EntityBase<table.TxLabelMap> {
    ...
    override get id(): number { throw new sdk.WERR_INVALID_OPERATION('entity has no "id" value') } // entity does not have its own id.
    override get entityName(): string { return 'TxLabelMap' }
    override get entityTable(): string { return 'tx_labels_map' }
    ...
    static async mergeFind(storage: sdk.WalletStorage, userId: number, ei: table.TxLabelMap, syncMap: entity.SyncMap, trx?: sdk.TrxToken)
    ...
    override async mergeExisting(storage: sdk.WalletStorage, since: Date | undefined, ei: table.TxLabelMap, syncMap: entity.SyncMap, trx?: sdk.TrxToken): Promise<boolean> {
        let wasMerged = false
